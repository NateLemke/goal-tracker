...
export default function ModalEditSubTask({closeFunc, editSubTaskFunc}) {
    const {subTaskDesc, subTaskProgress, subTaskID, taskID, goalID} = useTaskContext();

    const [description, setDescription] = useState(subTaskDesc);
    const [requiredProgress, setRequiredProgress] = useState(subTaskProgress);


    const editFunc = async() => {
        await editSubTaskFunc(goalID, taskID, subTaskID, description, requiredProgress);
        closeFunc();
    }
    ...
    return(
        ...
        justify-center items-center absolute">
            ...
            flex-col rounded-md min-w-60 bg-gray-700">
                ...
                <input id="descInput" type="text" placeholder="SubTask Description" 
